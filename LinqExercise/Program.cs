...
namespace LinqExercise
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            employees.Where(x => x.Age > 26).OrderByDescending(x => x.Age).ThenBy(x => x.FirstName).ToList()
                .ForEach(x => Console.WriteLine($"Name: {x.FullName}, Age{x.Age}" ));
            Console.WriteLine("--------------------------");

            //TODO: Print the Sum and then the Average of the employees' YearsOfExperience if their YOE is less than or equal to 10 AND Age is greater than 35
            var ans = employees.Where(x => x.Age > 35 && x.YearsOfExperience <= 10);
            Console.WriteLine($"Total YOE: {ans.Sum(x => x.YearsOfExperience)} Avg YOE: {ans.Average(x => x.YearsOfExperience)} ");
            Console.WriteLine("--------------------------");

            //TODO: Add an employee to the end of the list without using employees.Add()
            employees = employees.Append(new Employee("Colby", "Pate", 24, 2)).ToList();
